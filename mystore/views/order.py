...
class CartView(APIView):
    ...
    def post(self, request):
        if not IsBuyer().has_permission(request, self):
            return Response(
                {"detail": "Only buyers can add to cart."},
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        if not created:
            ...
            if product.available_quantity < (cart_item.quantity + quantity):
                return Response(
                    {
                        "detail": "Not enough stock to add more of this item to your cart."
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
...
class CheckoutView(APIView):
    def post(self, request):
        if not IsBuyer().has_permission(request, self):
            return Response(
                {"detail": "Only buyers can checkout."},
                status=status.HTTP_403_FORBIDDEN,
            )
...
class OrderHistoryView(APIView):
    def get(self, request):
        if not IsBuyer().has_permission(request, self):
            return Response(
                {"detail": "Only buyers can view their order history."},
                status=status.HTTP_403_FORBIDDEN,
            )
